...
// Simple in-memory cache for public info
let publicInfoCache = {
  data: null,
  lastUpdated: 0,
  ttl: 30000 // 30 seconds cache duration
};
...
app.get("/public/info", async (req, res) => {
  try {
    const now = Date.now();
    if (publicInfoCache.data && (now - publicInfoCache.lastUpdated < publicInfoCache.ttl)) {
      return res.json(publicInfoCache.data);
    }
    ...
    await sleep(100);
    const propertyAddress = await readContract.propertyAddress();
    await sleep(100);
    ...
    res.status(500).json({ error: "Failed to fetch info", details: err.toString() });
  }
});
...
app.get("/public/balance/:address", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch balance", details: err.toString() });
  }
});
...
app.get("/public/withdrawable-rent/:address", async (req, res) => {
  try {
    ...
    res.status(500).json({ error: "Failed to fetch withdrawable rent", details: err.toString() });
  }
});
